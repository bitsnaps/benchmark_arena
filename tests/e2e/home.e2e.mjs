...
// row-count expectations (which assume every row in the DOM) still hold
  ...
  if (navTexts.join(',') !== 'Leaderboard,Advisor,Benchmarks,Value map,Providers,Methodology') fail('navbar should be [Leaderboard, Advisor, Benchmarks, Value map, Providers, Methodology], got ' + navTexts);
  else ok('navbar: Leaderboard + Advisor + Benchmarks + Value map + Providers + Methodology (stats-45: Methodology last)');
